        ...
        with st.spinner("Loading"):
            ...
            for data in user_input_list:
                if "uploaded_files" in data and data["uploaded_files"]:
                    ...
                    if "Textract" in data["text_extraction"] and "GPT-4" in data["llm"]:
                        ...
                        with col1:
                            ai_summary = st.text_area("Here is the AI_SUMMARY", gpt_response, height=300, key=f"Text counter {generate_random_key(12)}")
                        

                        with col2:
                            gold_parameters = st.text_area("Enter Gold Parameters here", data["gold_std"], height=300, key=f"Text counter {generate_random_key(13)}")
            ...
            #                 st.text_area("Here is the evaluation", eval_gpt_response, height = 200, key = "012")
                    ...
                    if "Textract" in data["text_extraction"] and "Claude-2" in data["llm"]:      
                        ...
                        with col1:
                            ai_summary = st.text_area("Here is the AI_SUMMARY", claude_response, height=300, key=f"Text counter {generate_random_key(12)}")
                    ...
                    if "PDFMiner" in data["text_extraction"] and "GPT-4" in data["llm"]:
                            ...
                            col1,col2 = st.columns(2)
                            with col1:
                                ai_summary = st.text_area("Here is the AI_SUMMARY", gpt_response, height=500, key=f"Text counter {generate_random_key(12)}")
                            
                            with col2:
                                gold_parameters = st.text_area("Enter Gold Parameters here", data["gold_std"], height=500, key=f"Text counter {generate_random_key(13)}")
                    ...
                    if "PDFMiner" in data["text_extraction"] and "Claude-2" in data["llm"]:     
                        ...
                        col4,col5 = st.columns(2)
                        with col1:
                            ai_summary = st.text_area("Here is the AI_SUMMARY", claude_response, height=500, key=f"Text counter {generate_random_key(12)}")
                        with col2:
                            gold_parameters = st.text_area("Enter Gold Parameters here", data["gold_std"], height=500, key=f"Text counter {generate_random_key(13)}")
